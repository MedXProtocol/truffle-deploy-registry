...
describe('processAllArtifacts()', () => {
  ...
  beforeAll(async () => {
    ...
    contract4Path = path.join(outputArtifactsPath, 'Contract4.json')

    // touch the file to make sure it's there
    fs.writeFileSync(contract4Path, '')
  ...
  it('should update output file even if output already exists and artifact does not exist', async () => {
    await processAllArtifacts(inputArtifactsPath, outputArtifactsPath, networksPath)

    const json = await readFile(contract4Path)
