...
describe('processAllArtifacts()', () => {
  ...
  const sourceFilePath = path.join(artifactsPath, 'Contract2.json')

  beforeAll(async () => {
    outputPath = mkdirTmp()
    destinationFilePath = path.join(outputPath, 'Contract2.json')
  ...
  it('should process the artifacts', async () => {
    await processAllArtifacts(networksPath, outputPath)
